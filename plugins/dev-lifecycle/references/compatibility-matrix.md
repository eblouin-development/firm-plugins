...
| PyJWT | **2.13.x** (2.13.0) | Current stable (PyPI). HS256 JWT signing/verification for `templates/components/security/auth/_core.py`'s `TokenService` — access/refresh token issuance and decode, algorithm pinned explicitly to HS256 at every `jwt.decode()` call (never inferred from the token's own header — see that module's own docstring). First consumed by backend/fastapi's Stage 5a (#41) auth wiring; the Django track now also consumes the identical pin (backend/django's `pyproject.toml`), landed by Stage 5b (#44)'s auth vendoring — this is a framework-neutral runtime dependency of the component itself, not FastAPI-specific. |
| argon2-cffi | **25.1.x** (25.1.0) | Current stable (PyPI). Argon2id password hashing for `templates/components/security/auth/_core.py`'s `PasswordService` — the OWASP-recommended default for new applications (see that class's own docstring on why Argon2id specifically, not Argon2i/Argon2d). Same "first consumed by FastAPI's Stage 5a, Django track now also consumes the identical pin as of Stage 5b (#44)" note as PyJWT above applies here too. |
