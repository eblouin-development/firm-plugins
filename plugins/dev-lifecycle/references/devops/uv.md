<!--
library: uv
versions-covered: "uv 0.x / current CLI syntax — this kit's compatibility-matrix.md pins uv to 0.11.31 (Docker/security-tooling rows); latest available at last-verified is 0.11.32 (2026-07-23). CLI surface described here verified against 0.11.32 and cross-checked as compatible with the matrix's 0.11.31 pin — no breaking flag changes between the two."   # verified current: 0.11.32 (2026-07-23)
last-verified: 2026-07-24
provenance: manual
sources:
  # docs.astral.sh returned 403 to direct fetch this session; content verified via the same
  # docs source tree mirrored on GitHub (astral-sh/uv main branch) instead.
  - https://raw.githubusercontent.com/astral-sh/uv/main/README.md
  - https://raw.githubusercontent.com/astral-sh/uv/main/docs/concepts/projects/dependencies.md
  - https://raw.githubusercontent.com/astral-sh/uv/main/docs/concepts/projects/sync.md
  - https://raw.githubusercontent.com/astral-sh/uv/main/docs/concepts/projects/build.md
  - https://raw.githubusercontent.com/astral-sh/uv/main/docs/concepts/python-versions.md
  - https://raw.githubusercontent.com/astral-sh/uv/main/docs/pip/index.md
...
## Version check (do this first)
uv is pre-1.0 and ships often (current: **0.11.32**, released 2026-07-23; this kit's `compatibility-matrix.md` pins uv to **0.11.31** for Docker/security-tooling — one patch behind latest, a deliberate hold, not a stale pin). The CLI surface and `uv.lock` schema are versioned and still move, so **pin uv for reproducibility** — don't float `latest`. Check with `uv self version` (uv's own version; `uv version` reports/sets the *project* version). In CI pin via `astral-sh/setup-uv` `version:`; in Docker pin the `COPY --from=ghcr.io/astral-sh/uv:X.Y.Z` tag (this kit uses `0.11.31`, per the matrix) or the installer version. The lockfile records its own `version`; a newer uv may rewrite it, so keep local and CI uv in step.
...
## Dependency groups vs extras
- **`[dependency-groups]`** (PEP 735) — local/dev-only deps (test, lint, docs). Not published to PyPI, not installed by consumers. Select with `--group <name>`, `--all-groups`, `--only-group <name>`. `dev` is the special default group (`--dev`/`--no-dev`/`--only-dev`), included by `uv sync`/`uv run` automatically. This is the preferred default for tooling deps on new projects.
- **`[project.optional-dependencies]`** (extras) — optional *features* shipped to consumers, installed via `pip install pkg[extra]`. Add with `uv add --optional <extra> <pkg>`; select at sync/run time with `--extra <name>`/`--all-extras`. Use only for genuinely publishable optional features — if a project has extras that predate the PEP 735 group split, new dev tooling should still go in `[dependency-groups]` going forward rather than growing the extras list further.
...
## Docker
Pin uv by copying from the Astral image: `COPY --from=ghcr.io/astral-sh/uv:0.11.31 /uv /uvx /bin/` (this kit's pin — see `compatibility-matrix.md`; substitute your own pinned version). Set `ENV UV_COMPILE_BYTECODE=1` (compile `.pyc` at build for faster cold start) and `ENV UV_LINK_MODE=copy` (avoid cross-filesystem symlink warnings with cache mounts). Install deps before source for layer caching, using a cache mount:
```dockerfile
...
## Building & publishing
`uv build` produces sdist+wheel in `dist/`, delegating the actual build to whatever `build-backend` the project declares (e.g. hatchling). `uv publish` uploads (trusted publishing / token). These are lockfile-independent — the lock governs the dev env, not the built artifact's metadata.
...
- Floating uv version across dev/CI/Docker — pin it everywhere (this kit's own pin: uv **0.11.31**, per `references/compatibility-matrix.md`).
