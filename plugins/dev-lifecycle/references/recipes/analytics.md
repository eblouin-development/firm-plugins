...
## Wire-up steps (web: nextjs / vite-spa)
1. **Add a thin `analyticsApi.ts`** following the same adapter shape `authApi.ts`/`pushApi.ts` already establish in this kit's frontend blocks — a small wrapper over the generated `@repo/api-client` operation for `POST /analytics/events`, not a new HTTP client.
