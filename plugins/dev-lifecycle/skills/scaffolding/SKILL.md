...
### 6. Wire the pipeline
- **Issue and PR templates:** copy `${CLAUDE_PLUGIN_ROOT}/assets/pr/pull_request_template.md` to the repo's `.github/pull_request_template.md`, and `${CLAUDE_PLUGIN_ROOT}/assets/issue-templates/*.yml` to the repo's `.github/ISSUE_TEMPLATE/` (`feature.yml`, `bug.yml`, `epic.yml`, `config.yml`), substituting the `<owner>` placeholder with the repo owner wherever it appears. These give the repo GitHub-native forms that mirror what `planning` and `product-planning` file, so hand-filed and skill-filed issues/PRs stay consistent.
- **Claude GitHub App + workflows:** copy the firm's workflow templates from `${CLAUDE_PLUGIN_ROOT}/assets/workflows/` into the repo's `.github/workflows/`, replacing the `<owner>` placeholder with the marketplace repo owner. Two of the three are now **thin caller stubs**, not full workflows:
  - `claude.yml` (implement: responds to `@claude` on issues/PRs) and `claude-review.yml` (review: PR-opened → the review agent) define only the triggers and `uses:` the firm's **reusable workflows** hosted in the plugin marketplace repo's `.github/workflows/` (pinned to the moving major tag `@v1`). All the real logic — plugin load, prompt, tool allow-list, and the review's `@claude`/`@<owner>` routing — lives in those reusable workflows, so it is maintained **once** in the marketplace repo and every repo picks up improvements on the next marketplace-repo release, with no per-repo re-sync. Substitute `<owner>` and `<repo>` in the `uses:` path and the `owner:`/`repo:` inputs; the stub grants the job's permissions and forwards the OAuth secret via `secrets: inherit`.
...
- **The plugin is wired inside the reusable workflows** (`plugin_marketplaces: https://github.com/<owner>/<repo>.git` — a **full git URL**, not `owner/repo` shorthand, which the action rejects — plus `plugins: dev-lifecycle@<repo>`, with an appended system prompt telling the agent to actually **use** the firm skills, not just have them present). A locally-installed plugin does NOT reach the Action's runner, so this explicit load is what enables the firm's skills there. Two fetch requirements when the marketplace repo is **private**: (1) the runner's default `GITHUB_TOKEN` can't fetch the marketplace — make it public, or supply a read-scoped token (a secret) and configure git before the action; and (2) a caller stub can't `uses:` a private repo's reusable workflow unless the marketplace repo → Settings → Actions → General → Access permits this repo (or all repos owned by `<owner>`).
