 ...
 * (`/reset-password?token=…`). Port of the Vite SPA's `ResetPasswordPage.tsx`.
 * Like `/verify-email`, this reads `useSearchParams()` (from `next/navigation`
 * here, not react-router), which requires a `<Suspense>` boundary at build
 * time — the default export below wraps the token-reading screen in one.
 */
...
const ResetPasswordFallback = (): ReactNode => (
