...
/**
 * The admin area. Port of the Vite SPA's `AdminPage.tsx`. This route is
 * reached through the `<AdminGate>` client gate (RequireRole "admin"), but the
 * CLIENT gate is UX only — the AUTHORITATIVE gate is the server's 403 on
 * `GET /admin/ping`. So we call it and render BOTH branches: the 200 success
 * (the ping payload) AND the 403 "forbidden" answer, the latter being what a
 * user with a stale/forged `admin` claim actually hits.
 *
