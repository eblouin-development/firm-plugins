...
statement and returns a filled `Page`. SQLAlchemy 2.0 async, pinned per
...
sibling module (`from schema import Page, PageParams`), matching how the
rest of the backend/ catalog composes (see db-mixins/README.md's note on
keeping the SQLAlchemy-specific files together). SQLAlchemy-specific —
...
schema.py's `Page`/`PageParams` shape, reimplemented against a DRF
...
async def paginate_select(session: AsyncSession, stmt: Select[Any], params: PageParams) -> Page[Any]:
    ...
    truth for the offset math, not re-derived here."""
