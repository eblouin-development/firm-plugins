...
| `frontend/django-htmx` | `apps/api` (an overlay, NOT `apps/web`) | `backend/django` materialized into the same `apps/api`, its vendored `core.security.auth` — an **either/or alternative** to `frontend/vite-spa`/`frontend/nextjs` for the site's main UI (composes fine alongside `frontend/nextjs-admin` and the JSON API) | The `webapp/` app + templates/static overlay — server-rendered, HTMX-enhanced pages (`/login`, `/browse/items`, ...) in the same process as the JSON API |
...
| `infra/compose-host` | `infra/compose-host` | Built + pushed app image(s) in any registry, a single Docker host you own (home server or VPS, Tailscale optional), a domain (or tailnet) | Caddy-fronted TLS, Postgres with volume + backup job, self-hosted-runner or SSH deploy, restart-policy auto-recovery; graduates to `infra/aws-fargate` unmodified |
